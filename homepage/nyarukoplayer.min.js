...
function nyarukoplayer_animationinit(a){$.ajaxSetup({error:function(a,b){nyarukoplayer_consolelog&&console.error("Data loading failed: "+b);nyarukoplayer_error();return!1}});var c=nyarukoplayer_browserok();null!=c?nyarukoplayer_error(c):(nyarukoplayer_webpok=nyarukoplayer_checkWebpSupport(),$.isFunction($.cookie)&&"true"==$.cookie("disable")?(nyarukoplayer_consolelog&&console.warn("[NyarukoPlayer] Disabled by cookie."),$("#nyarukoplayer_loading").remove(),$("#titlebox").css("background","transparent")):
(nyarukoplayer_count=a.length,nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Configuration loaded."),$.isFunction(nyarukoplayerCallback_ConfigurationLoaded)&&nyarukoplayerCallback_ConfigurationLoaded(),nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Loading images..."),$.each(a,function(a,b){nyarukoplayer_webp&&nyarukoplayer_webpok&&(nyarukoplayer_imgtype="webp");imgurl=nyarukoplayer_imgdir+b[0]+"."+nyarukoplayer_imgtype;nyarukoplayer_width[a]=b[1];nyarukoplayer_height[a]=b[2];nyarukoplayer_from[a]=
b[3];nyarukoplayer_to[a]=b[4];nyarukoplayer_time[a]=b[5];nimg=new Image;nimg.src=imgurl;nimg.onload=function(){nyarukoplayer_imgcache[a]=$(this);nyarukoplayer_loaded++;var b=nyarukoplayer_loaded/nyarukoplayer_count*100;nyarukoplayer_consolelog&&console.log("Loading... "+nyarukoplayer_loaded+"/"+nyarukoplayer_count+" : "+b+"%");var c=$("#nyarukoplayer_loadingok");nyarukoplayer_errord||c.html("\u8f7d\u5165\u4e2d "+b.toFixed(0)+"%");c.css("width",b+"%");nyarukoplayer_loaded==nyarukoplayer_count&&(nyarukoplayer_consolelog&&
...
document.getElementById("nyarukoplayer_musiccontrol")&&!document.getElementById("nyarukoplayer_musiccontrol").paused&&(nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Music Pause."),$("#nyarukoplayer_audiodiv").css("animation","none"),document.getElementById("nyarukoplayer_musiccontrol").pause(),$.isFunction(nyarukoplayerCallback_MusicPause)&&nyarukoplayerCallback_MusicPause())}
...
function nyarukoplayer_unload(a){nyarukoplayer_unbindcallback();nyarukoplayer_stop();a&&$("#nyarukoplayer").remove()}
function nyarukoplayer_unbindcallback(){nyarukoplayerCallback_AnimateReady=nyarukoplayerCallback_MusicPause=nyarukoplayerCallback_MusicPlay=nyarukoplayerCallback_Error=nyarukoplayerCallback_MusicDisabled=nyarukoplayerCallback_MusicLanguageblock=nyarukoplayerCallback_LyricsLoaded=nyarukoplayerCallback_ImageLoaded=nyarukoplayerCallback_ConfigurationLoaded=nyarukoplayerCallback_LyricEnd=nyarukoplayerCallback_AnimateEnd=nyarukoplayerCallback_AnimateStart=null}
function nyarukoplayer_stop(){nyarukoplayer_musicdiglog_close();nyarukoplayer_playmusic(!1);nyarukoplayer_replay=!1;nyarukoplayer_animateend($(".nyarukodiv"),!0)}
...
d,b);c.css({left:l[0],top:l[1],width:l[2],height:l[3]});c.animate({left:d[0],top:d[1],width:d[2],height:d[3]},a,function(){nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] IMG "+(nyarukoplayer_now+1)+"/"+nyarukoplayer_count);nyarukoplayer_now<nyarukoplayer_count-1?(nyarukoplayer_now++,c.remove(),nyarukoplayer_play(!0)):nyarukoplayer_animateend(c,!1)})}
function nyarukoplayer_animateend(a,c){nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Animate End.");$.isFunction(nyarukoplayerCallback_AnimateEnd)&&nyarukoplayerCallback_AnimateEnd();if(nyarukoplayer_replay||c)nyarukoplayer_now=0,a.stop(),a.remove(),nyarukoplayer_play(nyarukoplayer_replay)}
...
function nyarukoplayer_audioinit(a){if($.isFunction($.cookie)&&"true"==$.cookie("disable"))nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Disabled by user."),$.isFunction(nyarukoplayerCallback_MusicDisabled)&&nyarukoplayerCallback_MusicDisabled();else{if(""!=nyarukoplayer_musicblock&&$.isFunction($.cookie)&&"true"!=$.cookie("nochkarea"))for(var c=navigator.language.substr(0,2),d=nyarukoplayer_musicblock.split("|"),b=0;b<d.length;b++)if(d[b]==c){nyarukoplayer_consolelog&&console.warn("[NyarukoPlayer] \u5bf9\u4e0d\u8d77\uff0c\u60a8\u6240\u5728\u7684\u5730\u533a\u4e0d\u80fd\u64ad\u653e\u8fd9\u9996\u80cc\u666f\u97f3\u4e50 \u3053\u306e\u30da\u30fc\u30b8\u306f\u304a\u4f4f\u307e\u3044\u306e\u5730\u57df\u304b\u3089\u3054\u5229\u7528\u306b\u306a\u308c\u307e\u305b\u3093 Cannot use this page from the area to live");
...
nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Lyrics loaded.");$.isFunction(nyarukoplayerCallback_LyricsLoaded)&&nyarukoplayerCallback_LyricsLoaded()}}}function nyarukoplayer_nochkarea(a){a=void 0===a?!1:a;$.isFunction($.cookie)&&$.cookie("nochkarea",a,{expires:365});location.reload(!1)}function nyarukoplayer_disable(a){a=void 0===a?!1:a;$.isFunction($.cookie)&&$.cookie("disable",a,{expires:365});location.reload(!1)}
...
function nyarukoplayer_musicdiglog_open(){if(0!=$("#nyarukoplayer_musicdiglog").length||nyarukoplayer_playd)return-1;if(!nyarukoplayer_lrcfile||!nyarukoplayer_musicfile)return nyarukoplayer_musicdiglog_nomusic(),2;var a=!!navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/);if(!a&&$.isFunction($.cookie)&&"1"==$.cookie("playmusic"))return setTimeout(function(){nyarukoplayer_playmusic(!0);nyarukoplayer_musicdiglog_close();nyarukoplayer_ready()},500),1;if(!a&&$.isFunction($.cookie)&&"2"==$.cookie("playmusic"))return nyarukoplayer_musicdiglog_nomusic(),
2;var c='<div id="nyarukoplayer_musicdiglog"><h1>\u8981\u5f00\u542f\u80cc\u666f\u97f3\u4e50\u5417\uff1f</h1><p><a id="nyarukoplayer_musicdiglog_yes">\u64ad\u653e(\u63a8\u8350)</a></p><p><a id="nyarukoplayer_musicdiglog_no">\u4e0d\u8981\u64ad\u653e</a></p>';a||(c+='<p><input type="checkbox" id="nyarukoplayer_musicdiglog_save" value="1" checked="checked" />\u4eca\u65e5\u4e0d\u518d\u8be2\u95ee</p>');c+="</div>";$("body").append(c);$("#nyarukoplayer_musicdiglog_yes").click(function(){!a&&$.isFunction($.cookie)&&
$("#nyarukoplayer_musicdiglog_save").prop("checked")&&$.cookie("playmusic","1",{expires:1});nyarukoplayer_playmusic(!0);nyarukoplayer_musicdiglog_close();nyarukoplayer_ready();nyarukoplayer_play()});$("#nyarukoplayer_musicdiglog_no").click(function(){!a&&$.isFunction($.cookie)&&$("#nyarukoplayer_musicdiglog_save").prop("checked")&&$.cookie("playmusic","2",{expires:1});nyarukoplayer_musicdiglog_close();nyarukoplayer_ready();nyarukoplayer_play()});return 3}
function nyarukoplayer_checkWebpSupport(){var a=document.createElement("canvas");return a.getContext&&a.getContext("2d")?0===a.toDataURL("image/webp").indexOf("data:image/webp"):!1}function nyarukoplayer_browserok(){var a=navigator.userAgent.toLowerCase(),c="";/ie/.test(a)&&(c="IE");/opera/.test(a)&&(c="Opera");return""==c?null:"\u62b1\u6b49,\u6682\u4e0d\u652f\u6301"+c+",\u5efa\u8bae\u4f7f\u7528\u6700\u65b0\u7248Chrome\u3002"};
